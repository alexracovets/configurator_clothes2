...
│   └── utils/              # Чисті функції: атласи, шейдери, конвертери файлів
...
- доменні дії (`useLogoFileHandler`).
...
├── ui/             # Props і допоміжні типи UI-компонентів
└── index.ts        # Barrel-експорт
```

#### Правила типів

1. **Entity-типи** (`GarmentConfig`, `NamePositionConfig`, `UvPoint`…) описують JSON у `src/data/` і живуть у `src/types/entities/`.
2. **Runtime-типи** (`NameInstance`, `LogoPosition`, `PartGradient`…) **успадковують або компонують** entity-типи через `Pick`, `Omit`, `extends` — **не дублюють** поля вручну.
3. Типи props компонентів — у `src/types/ui/`, не в `.tsx`-файлах.
4. `@data` експортує **лише дані й функції** (`getProduct`, `faqContent`…). Типи імпортуються напряму з `@types` — без проміжних `types.ts` у `data/`, `gizmo/` тощо.
...
- шейдери та uniform-и (`garmentPrint/`);
- конвертація логотипів (`logoFile/`);
- PBR-текстури (`pbrMaps/`).
...
| --------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `@constants`                | `DEFAULT_TEXT_CONFIGURATION`, `STEPS_CONFIGURATION`, `FONT_FAMILY_BY_NAME`… | —                                                                              |
| `@fonts`                    | `inter`, `anton`, `oswald`… (next/font instances)                           | `src/fonts/FONTS_CONFIGURATION.ts` (`'use client'`, залежить від `sportFonts`) |
...
@hooks (useGarmentTextures) → compose atlases
       ↓
@organisms/Configurator (R3F) → шейдери на mesh
       ↑
...
- [x] Runtime garment-типи (`NamePosition`, `LogoInstance`, `PartGradient`…) компонують entity-типи
...
- [ ] Усі props-типи компонентів у `src/types/ui/` (залишились у `.tsx`: `LogoUpload`, `AtomTabs`, `AtomSkeleton`, `PatternLayerColorControl`, `AtomRichText`, `AtomTable`)
...
- [ ] Utils-типи (`PbrMaps`, `GarmentPrintState`…) — перенести в `src/types/` або залишити як internal (допустимо для низькорівневих модулів)
- [ ] `useStepLogo` — React selector-hook у `store/`; кандидат на перенесення в `@hooks`
...
```ts
// entity (з JSON)
interface TextPositionConfig { label: string; uv: UvPoint; rotation: number; fontSize: number; ... }

// runtime — не дублює поля, а компонує
type TextPrintPosition = { key: string; partId: string; uv: UvPoint }
