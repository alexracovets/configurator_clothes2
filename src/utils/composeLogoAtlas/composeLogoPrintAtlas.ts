...
const isBackLogoPart = (instance: LogoInstance) =>
  instance.partId.includes('back') || instance.positionKey.toLowerCase().includes('back') || instance.label.toLowerCase().includes('back');

const resolveLogoDrawSize = (instance: LogoInstance, naturalWidth: number, naturalHeight: number, atlasWidth: number, atlasHeight: number) => {
...
const resolveLogoReferenceDrawSize = (instance: LogoInstance, naturalWidth: number, naturalHeight: number) =>
  resolveLogoDrawSize({ ...instance, scale: 1 }, naturalWidth, naturalHeight, LOGO_ATLAS_REF_WIDTH, LOGO_ATLAS_REF_HEIGHT);

const resolveLogoDisplayScale = (instance: LogoInstance, naturalWidth: number, naturalHeight: number, atlasWidth: number, atlasHeight: number) => {
...
export { composeLogoPrintAtlas, resolveLogoDisplayScale, resolveLogoDrawSize, resolveLogoReferenceDrawSize };
