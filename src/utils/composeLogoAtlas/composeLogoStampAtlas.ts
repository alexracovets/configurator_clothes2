...
interface LogoStampAtlas {
  canvas: HTMLCanvasElement;
  cellSize: { width: number; height: number };
}

interface ComposeLogoStampAtlasInput {
...
const resolveLogoStampCellSize = (instances: LogoInstance[], naturalById: Map<string, { width: number; height: number }>) => {
  let maxWidth = 1;
  let maxHeight = 1;
...
const composeLogoStampAtlas = async ({ instances, canvas }: ComposeLogoStampAtlasInput): Promise<LogoStampAtlas> => {
