...
import type { LogoStyleUniforms } from './buildLogoStyleUniforms';

interface GarmentLogoStampState {
  stamp: Texture;
...
let pendingLogoStamp: GarmentLogoStampState | null = null;
let pendingLogoStyle: LogoStyleUniforms | null = null;
let pendingLogoGizmoFrame: GizmoFrameState | null = null;

const applyLogoStampToUniforms = (material: MeshStandardMaterial, state: GarmentLogoStampState) => {
...
const applyLogoStyleToUniforms = (material: MeshStandardMaterial, style: LogoStyleUniforms) => {
...
const applyLogoGizmoFrameToUniforms = (material: MeshStandardMaterial, state: GizmoFrameState) => {
...
const applyGarmentLogoStamp = (material: MeshStandardMaterial, state: GarmentLogoStampState) => {
  pendingLogoStamp = state;
  material.userData.garmentLogoStampState = state;
  applyLogoStampToUniforms(material, state);
};

const applyGarmentLogoStyle = (material: MeshStandardMaterial, style: LogoStyleUniforms) => {
  pendingLogoStyle = style;
  material.userData.garmentLogoStyleState = style;
  applyLogoStyleToUniforms(material, style);
};

const applyGarmentLogoGizmoFrame = (material: MeshStandardMaterial, state: GizmoFrameState) => {
...
const hydrateGarmentLogoUniforms = (
  ...
  const stampState = (material.userData.garmentLogoStampState as GarmentLogoStampState | undefined) ?? pendingLogoStamp;
  const styleState = (material.userData.garmentLogoStyleState as LogoStyleUniforms | undefined) ?? pendingLogoStyle;
  const gizmoState = (material.userData.garmentLogoGizmoFrameState as GizmoFrameState | undefined) ?? pendingLogoGizmoFrame;
...
export type { GarmentLogoStampState, GizmoFrameState };
