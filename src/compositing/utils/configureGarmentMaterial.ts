...
const configureGarmentMaterial = (material: MeshStandardMaterial, bounds: UvBounds, maps: PbrMaps, partKey: Uv0PartKey | null) => {
  ...
  if (!material.userData.garmentShaderConfigured) {
    ...
    material.customProgramCacheKey = () => `garment-part-${partKey ?? 'x'}-${bounds.minX}-${bounds.minY}-${bounds.maxX}-${bounds.maxY}`;
  } else {
